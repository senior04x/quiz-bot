...
const allData = loadAllTests();
const express = require('express');
const app = express();
const port = process.env.PORT || 3000;
app.get('/', (req, res) => res.send('Bot is running!'));
app.get('/ping', (req, res) => res.send('pong'));
app.listen(port, () => console.log(`Web server is listening on port ${port}`)); // Load on startup
...
bot.onText(/\/start(?:\s+(.+))?/, (msg, match) => {
    ...
    if (payload) {
        ...
        if (parts.length >= 3) {
            ...
            if (subjectFile === 'test6.md') subjectName = "Global iqtisodiyot";
    ...
    const options = {
        reply_markup: {
            inline_keyboard: [
                ...
                [{ text: '📚 Global iqtisodiyot', callback_data: 'sub_test6.md' }]
            ]
        }
    };
...
function finishTest(chatId, session) {
    ...
    if (session.subjectFile === 'test6.md') subjectName = "Global iqtisodiyot";
